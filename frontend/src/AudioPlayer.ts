...
export class AudioPlayer {
  ...
  reset() {
    ...
    console.log(
      `[AudioPlayer] Reset called, new generation: ${this.currentGeneration}`
    );
  ...
  private stopAllAudio() {
    console.log(
      `[AudioPlayer] Stopping ${this.activeSources.size} active audio sources`
    );
  ...
  async addToQueue(url: string, text: string) {
    ...
    console.log(
      `[AudioPlayer] Adding to queue - Generation: ${generation}, URL: ${url}`
    );
  ...
  private async processQueue(generation: number) {
    ...
    console.log(
      `[AudioPlayer] Starting queue processing for generation ${generation}`
    );
    ...
    while (this.playbackQueue.length > 0) {
      ...
      if (generation !== this.currentGeneration) {
        console.log(
          `[AudioPlayer] Generation mismatch (${generation} vs ${this.currentGeneration}), stopping queue processing`
        );
        break;
      }
  ...
  private async playAudio(
    ...
    if (generation !== this.currentGeneration) {
      console.log(
        `[AudioPlayer] Skipping outdated audio from generation ${generation}`
      );
      return;
    }

    console.log(
      `[AudioPlayer] Playing audio: ${url} (${text.substring(0, 50)}...)`
    );
    ...
    try {
      ...
      if (generation !== this.currentGeneration) {
        console.log(
          `[AudioPlayer] Generation changed during fetch, skipping playback`
        );
        return;
      }
      ...
      if (generation !== this.currentGeneration) {
        console.log(
          `[AudioPlayer] Generation changed during decode, skipping playback`
        );
        return;
      }
  ...
  unlock() {
    ...
    this.audioContext
      ...
      .then(() => {
        ...
        console.log(
          "[AudioPlayer] AudioContext resumed, state:",
          this.audioContext!.state
        );
